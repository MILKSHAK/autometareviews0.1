...
class TextPreprocessing
  ...
  def segment_text(flag, text_array)
    ...
    for k in (0..text_array.length-1)
      ...
      if (text.include?(".") or text.include?("?") or text.include?("!") or text.include?(",") or text.include?(";") )
        # the text contains more than 1 sentence
        while (text.include?(".") or text.include?("?") or text.include?("!") or text.include?(",") or text.include?(";")) do
          endd = 0
          ...
          if ((text.include?("?") and endd != 0 and endd > text.index("?")) or (text.include?("?") and endd == 0))
            endd = text.index("?")
          end
          ...
          # if a , occurs before any of . or ? or !
          if ((text.include?(",") and endd != 0 and endd > text.index(",")) or (text.include?(",") and endd == 0))
            endd = text.index(",")
          end
          # if a ; occurs before any of . or ?, ! or ,
          ...
          # check if the string between two commas or punctuations is there to
          # buy time e.g. ", say," ",however," ", for instance, "...
          if (flag == 0)
            # training
          ...
          # incrementing the sentence counter
          i += 1
          # from end+1 to the end of the string variable
          text = text[(endd+1)..text.length]
        end
      else
        if (flag == 0)
          # training
      ...
      # incrementing reviews counter only for test reviews
      if (flag == 1)
        j += 1
      end
    end
  ...
  def remove_text_within_quotes(review_text)
    ...
    review_text.each{ |row|
      ...
      # set the text after all quoted segments have been removed.
      reviews << text
    }
    # return only the first array element - a string!
    return reviews
  end

  # Looks for spelling mistakes in the text and fixes them using the
  # raspell library available for ruby
